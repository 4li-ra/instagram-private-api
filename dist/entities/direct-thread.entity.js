...
class DirectThreadEntity extends entity_1.Entity {
  ...
  async broadcastVideo(options) {
    ...
    await Bluebird.try(() =>
      ...
      errors_1.IgResponseError,
      publish_service_1.PublishService.catchTranscodeError(videoInfo, options.transcodeDelay || 4 * 1000),
    );
    return await this.broadcast({
  ...
  async broadcastVoice(options) {
    ...
    await Bluebird.try(() =>
      ...
      errors_1.IgResponseError,
      publish_service_1.PublishService.catchTranscodeError({ duration }, options.transcodeDelay || 4 * 1000),
    );
    return await this.broadcast({
      item: 'share_voice',
      form: {
  ...
  async broadcastStory(input) {
    ...
    const baseOptions = Object.assign(Object.assign({}, options), {
      viewMode: options.viewMode || 'replayable',
      replyType: options.replyType,
    });
    if (this.threadId !== null) {
      return await this.client.publish.story(
        Object.assign(Object.assign({}, baseOptions), { threadIds: [this.threadId] }),
      );
    }
    if (this.userIds !== null) {
      return await this.client.publish.story(
        Object.assign(Object.assign({}, baseOptions), { recipientUsers: this.userIds }),
      );
    }
